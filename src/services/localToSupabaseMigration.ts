 ...
 * for the currently logged-in user. Safe to call repeatedly — only runs once
 * per device thanks to a flag stored in Preferences.
 */
export const migrateLocalStorageToSupabase = async (userId: string) => {
  ...
  if (bmi.length) {
    inserts.push(
      supabase.from('bmi_entries').insert(
        bmi.map((r) => ({
          user_id: userId,
          height: r.height,
          weight: r.weight,
  ...
  if (sleep.length) {
    inserts.push(
      supabase.from('sleep_entries').insert(
        sleep.map((s) => ({
          user_id: userId,
  ...
  if (hydration.length) {
    inserts.push(
      supabase.from('hydration_entries').insert(
        hydration.map((h) => ({
          user_id: userId,
          amount_ml: h.amount,
